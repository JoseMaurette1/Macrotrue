...
const MealData = () => {
  ...
  return (
    ...
    >
      {Object.entries(mealData).map(([category, meals]: [string, Meal[]]) => {
        const meal = meals[mealIndexes[category as keyof typeof mealIndexes]];
        return (
          <div key={category} className="space-y-4">
            <h2 className="text-3xl font-bold tracking-tight capitalize ">{category}</h2>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="">{meal.macros.calories}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="font-medium ">Protein:</span>
                      <span className="">{meal.macros.protein}g</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="font-medium ">Carbs:</span>
                      <span className="">{meal.macros.carbs}g</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="font-medium ">Fat:</span>
                      <span className="">{meal.macros.fat}g</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
